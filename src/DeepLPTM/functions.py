...
def training_graph_vectorization(adj_label, features, edges, optimizer, etm, deeplpm, args,
    ...
    Arguments:
        ...
        - init      : string
                    either 'random', 'kmeans', 'true_init' (requires to provide labels) (default = 'random'),
                    'dissimilarity' (requires to provide adjacency matrix as array)
                    'load' requires to provide 'init_path'
        - init_path : (opt) str
    ...
    """
    assert init in ['random', 'true_init', 'dissimilarity', 'load', 'deeplpm'], "init should be either 'random', 'load' 'dissimilarity', 'deeplpm' or 'true_init'"
    assert use in ['all', 'texts', 'network'], "all sould either be one of 'all', 'texts' or 'network'"
